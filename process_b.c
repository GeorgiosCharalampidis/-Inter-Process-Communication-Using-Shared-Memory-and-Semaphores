...
void* send_message(void* arg) {
    ...
    while (1) {
        ...
        shm->complete = 1;  // Indicate that the message segmentation is complete
        if (debug_mode) {
            printf("--Message sent from process B to process A\n");
        }
        sem_post(&shm->sem2);
        shm->message_A_to_B = 0;
        shm->message_B_to_A = 1;
...
void* receive_message(void* arg) {
    ...
    while (1) {
        ...
        gettimeofday(&start, NULL);  // Start the timer before waiting for a new message

        sem_wait(&shm->sem2);

        if (shm->complete && shm->message_B_to_A == 0) {
            if (!first_segment_received) {
                gettimeofday(&end, NULL);  // Stop the timer after receiving the first segment
                if (debug_mode) {
            ...
            if (debug_mode) {
                printf("--Message received from process A to process B\n");
            }
